...
def plot_durations(save_name=None):
    ...
    if save_name is not None:
        plt.savefig(
            project_path +
            '/exercise1_CartPole_v0/experiment6_part2/Duration_balance_pole_episode{}.jpg'.format(
                save_name))
        return None
...
class memory_container():
    ...
    def __init__(self, memory_capacity):
        self.best_memory_cache = []
        self.memory_capacity = memory_capacity

    def update_memory(self, one_episode_sample):
        if len(self.best_memory_cache) < self.memory_capacity:
            self.best_memory_cache.append((len(one_episode_sample), one_episode_sample))
        else:
            self.best_memory_cache = sorted(self.best_memory_cache, key=lambda x: x[0])
            if len(one_episode_sample) > self.best_memory_cache[0][0]:
                self.best_memory_cache.pop(0)
                self.best_memory_cache.append((len(one_episode_sample), one_episode_sample))

    def reuse_memory(self):
        best_memory = pd.DataFrame(self.best_memory_cache)[1]
        return_memory_num = self.memory_capacity if self.memory_capacity <= \
            len(self.best_memory_cache) else len(self.best_memory_cache)
        return best_memory.sample(return_memory_num).tolist()


best_memory_container = memory_container(40)
for i_episode in range(num_episodes):
    ...
    best_memory_container.update_memory(one_episode_sample)
    optimize_model(one_episode_sample)
    if (i_episode + 1) % 40 == 0:
        for best_memory in best_memory_container.reuse_memory():
            optimize_model(best_memory)
