...
class DMC(nn.Module):
    ...
    def __init__(self, h, w, outputs):
        ...
        self.linear1 = nn.Linear(linear_input_size, 2)
        self.bnl1 = nn.BatchNorm1d(2)
        self.linear2 = nn.Linear(4, 3)
        self.bnl2 = nn.BatchNorm1d(3)
        self.linear3 = nn.Linear(3, outputs)

    # Called with either one element to determine next action, or a batch
    # during optimization. Returns tensor([[left0exp,right0exp]...]).
    def forward(self, x):

        state_pic, state_env = x

        state_pic = F.relu(self.bn1(self.conv1(state_pic)))
        state_pic = F.relu(self.bn2(self.conv2(state_pic)))
        state_pic = F.relu(self.bn3(self.conv3(state_pic)))

        pic_output = F.elu(self.bnl1(self.linear1(state_pic.view(state_pic.size(0), -1))))
        last_input = torch.cat((pic_output, state_env), dim=1)
        last_output = F.elu(self.bnl2(self.linear2(last_input)))
...
def get_screen():
    ...
    return resize(screen).unsqueeze(0).to('cpu'), origin_screen
...
def select_action(state):
    state_pic, state_env = state
    state_pic = state_pic.to(device)
    state_env = torch.tensor([state_env], dtype=torch.float32).to(device)
    global steps_done
    ...
    if sample > eps_threshold:
        ...
        with torch.no_grad():
            ...
            action_tensor = cnn_net((state_pic, state_env)).max(1)[1].view(1, 1).to('cpu').tolist()
        cnn_net.train()
...
def plot_durations(save_name=None):
    ...
    if save_name is not None:
        plt.savefig(
            project_path +
            '/exercise1_CartPole_v0/experiment9_part3/Duration_balance_pole_episode{}.jpg'.format(
                save_name))
        return None
...
def optimize_model(one_episode_sample):
    ...
    :param one_episode_sample: 是字典, 包含键 'state_pic', 'state_env', 'action', 'reward'
    state_pic: torch.tensor, shape = (episode_length, C, H, W)
    state_env: torch.tensor, shape = (episode_length, 2)
    ...
    state_pic = one_episode_sample['state_pic'].to('cpu')
    state_env = one_episode_sample['state_env'].to('cpu')
    action = one_episode_sample['action'].to(torch.int64).unsqueeze(1).to('cpu')
    ...
    for i in range(len(action) - 1, -1, -1):
        ...
        all_G.append(G)
    all_G.reverse()
    all_G = torch.tensor(all_G, dtype=torch.float32).unsqueeze(1).to('cpu')

    index = list(range(len(action)))
    index = random.sample(index, len(index))
    state_pic = state_pic[index]
    state_env = state_env[index]
    action = action[index]
    all_G = all_G[index]
    batch_size = 300
    while len(state_pic) % batch_size == 1:
        batch_size -= 1
    for i in count():
        if i > 0:
            last_i = i * batch_size
            first_i = (i - 1) * batch_size
            tmp_state_pic = state_pic[first_i: last_i].to(device)
            tmp_state_env = state_env[first_i: last_i].to(device)
            tmp_action = action[first_i: last_i].to(device)
            tmp_all_G = all_G[first_i: last_i].to(device)
            if len(tmp_state_pic) == 0:
                break
            # 依然注意动作价值函数是(a|s)的价值，是对在某特定状态下执行特定动作的评价
            state_action_values = cnn_net((tmp_state_pic, tmp_state_env)).gather(1, tmp_action)
            loss = F.smooth_l1_loss(state_action_values, tmp_all_G)
            # Optimize the model
            optimizer.zero_grad()
            loss.backward()
...
for i_episode in range(num_episodes):
    ...
    env_state = env.reset().tolist()
    state_env = env_state[0: 2]
    last_screen, _ = get_screen()
    current_screen, origin_screen = get_screen()
    state_pic = current_screen - last_screen
    one_episode_sample = {
        'state_pic': [],
        'state_env': [],
        'action': [],
    ...
    for t in count():
        ...
        action = select_action((state_pic, state_env))
        one_episode_sample['state_env'].append(state_env)
        env_state, reward, done, _ = env.step(action)
        state_env = env_state[0: 2]
        one_episode_sample['state_pic'].append(state_pic)
    ...
    one_episode_sample['state_pic'] = torch.cat(one_episode_sample['state_pic'], dim=0)
    one_episode_sample['state_env'] = torch.tensor(one_episode_sample['state_env'], dtype=torch.float32)
